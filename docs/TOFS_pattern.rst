...
If we want to cover the possibility of a special template for a minion identified by ``intellij01`` then we could have a specific template in ``/srv/saltstack/salt/ntp/files/intellij01/etc/ntp.conf.jinja``.

.. code-block:: jinja

   {#- /srv/saltstack/salt/ntp/files/intellij01/etc/ntp.conf.jinja #}
   {#- Managed by saltstack #}
   {#- Edit pillars or override this template in saltstack if you need customization #}

   {#- Some crazy configurations here for intellij01 #}
   {#- ... #}
